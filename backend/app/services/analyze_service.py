...
def analyze_video_from_path(
    ...
    try:
        ...
        # 회전 메타데이터가 없고 세로 영상(H > W)인 경우, 모바일 촬영 패턴 감지
        # 모바일에서 세로 촬영 시 프레임 데이터가 가로로 저장되어 있을 수 있음
        if rotation_angle == 0 and H > W:
            # 첫 프레임을 읽어서 실제 프레임 크기 확인
            ret, test_frame = cap.read()
            if ret:
                frame_h, frame_w = test_frame.shape[:2]
                # 보고된 크기와 실제 프레임 크기가 다르면 회전 필요
                # 모바일 세로 영상: 보고된 크기 384x640, 실제 프레임 640x384 → 90도 회전 필요
                if frame_w > frame_h and W < H:
                    # 실제 프레임이 가로인데 보고된 크기는 세로 → 90도 시계방향 회전 필요
                    rotation_angle = 90
                    print(f"📐 모바일 세로 영상 감지: 보고된 크기 {W}x{H}, 실제 프레임 {frame_w}x{frame_h} → 90도 회전 적용")
