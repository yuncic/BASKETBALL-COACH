...
### 아쉬운점
1. 유저별로 로그인 기능도 접목해서 유저의 슈팅 영상을 저장을 받고 계속 사용자 맞춤형으로 도움을 주는 앱을 만들고 싶었음
	하지만, 시간도 부족하고 영상을 저장하기 위해선 별도의 스토리지서버도 확보를 해야하기에 다음 기회에 기능 확장을 도전할 것임.

2. 슛폼 분석 코드는 도저히 책임별, 기능별로 나누기 힘들었음 너무 어려운 내용이라 구현만으로도 빡셌음 

3. 슛폼 분석 로직은 정해진 시간안에 이론적 한계를 극복할 수 없다고 판단, 바이브 코딩으로 AI의 힘을 빌림 
    -> 추후 기능확장시에는 이론 공부를 해서 직접 모든 로직을 구현할 수 있도록 해볼 것
